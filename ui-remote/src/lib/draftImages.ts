/**
 * 草稿图片处理：解码、压缩归一化、生成预览 URL。
 *
 * 与桌面端 ui/src/features/chat/draftImages.ts 保持一致的约束（类型、尺寸、体积），
 * 保证两端产出的附件规格统一。远端图片经 base64 走 WS 上传，故仍做压缩控制体积。
 */
...
/** 把 Blob 转为 base64 字符串（去掉 data URL 前缀），供 WS 上传。 */
