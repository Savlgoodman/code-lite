...
import { createImageJsonHttp, downloadImage, imageJsonHttp } from "./imageHttp";
...
export function startGeneration(recordId: string, params: ImageRunParams): void {
  ...
  generationControllers.set(recordId, controller);
  genErrors.delete(recordId);
  emitGen();
  void (async () => {
    try {
...
export async function runGeneration(
  ...
  try {
    ...
    await imageBlobStore.deleteMany(imageIds).catch(() => undefined);
    throw error;
  }
}
