/**
 * 文件变更 diff 的纯逻辑：从工具调用提取 fileDiff 摘要、推导行级 diff、统计增删。
 *
 * 从 ui/src/features/chat/fileDiffs.ts 抽出，供桌面与远端复用。
 * 不依赖 React，不产生副作用。
 */

import type { FileDiffSummary, ToolCallItem } from "@code-lite/protocol";
